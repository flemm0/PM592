...
anova(
  ...
  lm(glucose ~ alcohol_frequency.f * diet_health + alcohol_frequency.f * sleep_hrs + bmi_cat * met_log , data = nhanes)
) # alcohol frequency and sleep hours interaction not significant (p = 0.77)

anova(
  lm(glucose ~ alcohol_frequency.f * diet_health + alcohol_frequency.f * sleep_hrs + bmi_cat + met_log , data = nhanes),
  lm(glucose ~ alcohol_frequency.f * diet_health + alcohol_frequency.f * sleep_hrs + bmi_cat * met_log , data = nhanes)
) # bmi and met activity score significant (p = 0.097)
```
...
tab_model(
  lm(glucose ~ diet_health + sleep_hrs + bmi_cat * met_log , data = nhanes),
  lm(glucose ~ diet_health + sleep_hrs + bmi_cat * met_log + age, data = nhanes),
  lm(glucose ~ diet_health + sleep_hrs + bmi_cat * met_log + gender, data = nhanes),
  lm(glucose ~ diet_health + sleep_hrs + bmi_cat * met_log + age + gender, data = nhanes),
  terms = c("diet_health", "sleep_hrs", "met_log"),
  dv.labels = c("Unadjusted", "Age Adjusted", "Gender Adjusted", "Age and Gender Adjusted")
)
```

```{r adjusted models output}
# full output for display
pred.labels <- c("(Intercept)", "Diet Health Score", "Sleep Hours", "Underweight", "Overweight", "Obese", "log(MET)",
                "Underweight:log(MET)", "Overweight:log(MET)", "Obese:log(MET)", "Age", "Gender")
...
```{r}
m1<- lm(glucose ~ diet_health + sleep_hrs + bmi_cat * met_log + age + gender, data = nhanes)
summary(m1)
```
...
m2 <- lm(glucose_log ~ diet_health + sleep_hrs + bmi_cat * met_log + age + gender, data = nhanes)
...
ols_plot_dffits(m2, print_plot = FALSE)
#ols_plot_dfbetas(m2, print_plot = FALSE)
```
...
```{r}
cbind(
  pred = predict(m2, newdata = nhanes[c(3396, 2519, 2460),]),
  nhanes[c(3396, 2519, 2460), c(ncol(nhanes), seq_along(names(nhanes)[-ncol(nhanes)]))]
)

```

The data points look reasonable, there is no compelling reason to remove them from the model.
...
```{r}
sim_slopes(m2, pred = met_log, modx = bmi_cat)
interact_plot(m2, pred = met_log, modx = bmi_cat)
```
